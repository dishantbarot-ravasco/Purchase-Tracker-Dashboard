"""
Characterization tests for apps/api/routers/vapi_views.py's
...
byte-for-byte across all three plants) - this file closes that gap for Vapi
specifically, ahead of the refactor that will make all three share one
implementation.
"""
...
def _make_po_mir_match():
    po = RTPVapiPurchaseOrder.objects.create(
        po_drive_folder_name="2000009999", po_number="2000009999", vendor_name="Test Vendor Ltd",
    )
    item = RTPVapiPOLineItem.objects.create(
    ...
    return RTPVapiPOMirMatch.objects.create(
...
def _make_mir_stock_match():
    mir = RTPVapiMIREntry.objects.create(mir_no="MIR-2", party_name="Vendor B", material_description="Gadget", source_row_ref="11")
    lot = RTPVapiStockLot.objects.create(description="Gadget", supplier_name="Vendor B", source_row_ref="20")
    return RTPVapiMirStockMatch.objects.create(mir_entry=mir, stock_lot=lot, rate_diff_pct="8.00", is_flagged=True)


@pytest.mark.django_db
class TestVapiDismissPoMirMatch:
    def setup_method(self):
        self.match = _make_po_mir_match()
        self.url = f"/api/vapi/matches/po-mir/{self.match.id}/dismiss"
    ...
    def test_unknown_match_id_returns_404(self):
        ...
        response = client.patch("/api/vapi/matches/po-mir/999999/dismiss", {"dismissed": True}, format="json")
...
@pytest.mark.django_db
class TestVapiDismissMirStockMatch:
    def setup_method(self):
        self.match = _make_mir_stock_match()
        self.url = f"/api/vapi/matches/mir-stock/{self.match.id}/dismiss"
