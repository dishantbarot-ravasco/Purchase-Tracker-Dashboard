"""
Characterization tests for apps/api/routers/vapi_views.py's correct_field -
...
exactly, pointed at Vapi's own models/URL prefix, since vapi_views.py had no
direct test coverage of its own before this file - its only prior indirect
coverage was via imports_views.py's tests (a different router) and the
Achhad-only material-field assertion in test_material_correct_field.py.
"""
...
class TestVapiCorrectField:
    def setup_method(self):
        self.po = _make_po()
        self.item = _make_item(self.po)
        self.url = f"/api/vapi/purchase-orders/{self.po.po_number}/fields"
    ...
    def test_editor_scoped_to_this_plant_can_correct(self):
        client = APIClient()
        client.force_authenticate(user=make_user(email="e6@ravasco.com", role="editor", plants=["vapi"]))
