...
class TestHrsCorrectField:
    ...
    def test_invalid_decimal_value_returns_400_not_500(self):
        # Regression test: Decimal(str(raw_value)) raises decimal.InvalidOperation
        # (an ArithmeticError, not a ValueError) for non-numeric input, which
        # _coerce_value's caller only caught as (TypeError, ValueError) - this
        # used to crash with an unhandled 500 instead of a clean 400. Fixed by
        # translating InvalidOperation into ValueError in _coerce_value itself.
        client = APIClient()
    ...
    def test_rematch_trigger_field_does_not_error_with_no_mir_data(self):
        # qty is in _REMATCH_TRIGGER_FIELDS - this exercises run_full_match()
        # actually being invoked, against an otherwise-empty HRS dataset.
        client = APIClient()
