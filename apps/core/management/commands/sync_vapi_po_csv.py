...
Usage:
    ...
    python manage.py sync_vapi_po_csv --file path.csv  # parse a local file instead (dev/testing)
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            with transaction.atomic():
                ...
                deactivated = deactivate_missing_orders(RTPVapiDomesticPurchaseOrder, orders)

            self._sync_data_quality_flags()
            self._report_orphans(orders)

            self.stdout.write(self.style.SUCCESS(
                f"sync_vapi_po_csv: {rows_seen} POs seen, {rows_changed} created/updated, "
            ...
            self.stderr.write(self.style.ERROR(f"sync_vapi_po_csv: header mismatch - {exc}"))
        except Exception as exc:
            status = SyncRun.Status.FAILED
            error_detail = str(exc)
            self.stderr.write(self.style.ERROR(f"sync_vapi_po_csv: failed - {exc}"))
        finally:
    ...
    def _load_csv_text(self, local_path: str | None) -> str:
        ...
        file_id = find_file_id_by_title(settings.VAPI_PO_CSV_TITLE, parent_id=settings.PURCHASE_TRACKER_DB_FOLDER_ID)
    ...
    def _report_orphans(self, parsed_orders) -> None:
        ...
        orphans = orphaned_orders(RTPVapiDomesticPurchaseOrder, parsed_orders)
        if not orphans:
            return
        shown = ", ".join(sorted(orphans)[:5])
        more = f" (+{len(orphans) - 5} more)" if len(orphans) > 5 else ""
        self.stdout.write(self.style.WARNING(
            f"sync_vapi_po_csv: {len(orphans)} stored PO(s) are no longer in the master CSV "
