...
Thin by design, exactly like match_vapi.py beside it: all the arithmetic
...
Records a `SyncRun.Source.CONSUMPTION` row for the same reason match_vapi.py
...
Vapi has no daily Recp./Issue matrix (Achhad is the only plant whose
Stock file carries one), so this reads snapshot intervals only.

Usage:
    python manage.py compute_vapi_consumption              # trailing lookback
    python manage.py compute_vapi_consumption --all        # whole history
    python manage.py compute_vapi_consumption --since 2026-09-01
"""
...
_PLANT_KEY = "vapi"
_PLANT = SyncRun.Plant.RTP_VAPI


class Command(BaseCommand):
    help = "Rebuild the RTP-Vapi raw-material consumption ledger from synced stock snapshots."
