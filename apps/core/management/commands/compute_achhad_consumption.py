"""
apps/core/management/commands/compute_achhad_consumption.py — rebuilds RTP-Achhad's
raw-material consumption ledger (MaterialConsumptionDaily +
ConsumptionEvent) from the *RMSnapshot history already in Postgres.

Thin by design, exactly like match_achhad.py beside it: all the arithmetic
...
Records a `SyncRun.Source.CONSUMPTION` row for the same reason match_achhad.py
...
Achhad's ledger also folds in RTPAchhadRMDailyMovement, the Stock file's
own daily Recp./Issue matrix - a genuinely dated per-day source that is
preferred over snapshot intervals wherever it reaches. See
consumption_ledger._dated_movements() for why (its dates are the real
issue dates; a snapshot lags them by a day).

Usage:
    python manage.py compute_achhad_consumption              # trailing lookback
    python manage.py compute_achhad_consumption --all        # whole history
    python manage.py compute_achhad_consumption --since 2026-09-01
"""
...
_PLANT_KEY = "achhad"
_PLANT = SyncRun.Plant.RTP_ACHHAD


class Command(BaseCommand):
    help = "Rebuild the RTP-Achhad raw-material consumption ledger from synced stock snapshots."
